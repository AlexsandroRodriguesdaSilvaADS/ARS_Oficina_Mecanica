// CÓDIGO OTIMIZADO 26/66 16:27
let somaTotal = 0;

// Centralização das referências do DOM (IDs atualizados para bater com o padrão mais comum)
const El = {
    ...
    // IMPORTANTE: Mapeando os novos campos buscando por IDs comuns
    // Se no seu HTML o ID for diferente, ajuste aqui à direita:
    selectFormaPagamento: document.getElementById('formaPagamento') || document.getElementById('f_pagamento') || document.getElementById('selectFormaPagamento'),
    inputDesconto: document.getElementById('desconto') || document.getElementById('f_desconto') || document.getElementById('inputDesconto')
};
...
function configurarEventos() {
    ...
    // Escutador de evento para atualizar o valor quando mudar o serviço
    if (El.selectServico) {
        El.selectServico.addEventListener('change', function () {
            const opcaoSelecionada = this.options[this.selectedIndex];
            const valorDoServico = opcaoSelecionada.getAttribute('data-valor');
            if (El.inputValor) El.inputValor.value = valorDoServico ? valorDoServico : '';
        });
    }

    // Gerencia a ativação do input de desconto com base na forma de pagamento (Dinheiro ou Pix)
    if (El.selectFormaPagamento && El.inputDesconto) {
        El.selectFormaPagamento.addEventListener('change', function () {
            if (this.value === 'dinheiro' || this.value === 'pix') {
                El.inputDesconto.disabled = false;
                El.inputDesconto.value = '5'; // Sugere 5% por padrão
            } else {
                El.inputDesconto.disabled = true;
                El.inputDesconto.value = '';
            }
        });
    }
}
...
function adicionarServico() {
    // PROTEÇÃO: Avisa se os elementos básicos sumiram do HTML
    if (!El.selectServico || !El.inputValor || !El.listaServicos || !El.inputTotalGeral) {
        alert("Erro técnico: Alguns elementos básicos de serviços não foram encontrados na página.");
        return;
    }

    if (!El.selectFormaPagamento || !El.inputDesconto) {
        alert("Erro técnico: O campo de Forma de Pagamento ou o campo de Desconto não foram encontrados no HTML. Verifique os IDs.");
        return;
    }

    const nomeServico = El.selectServico.value;
    const valorOriginal = parseFloat(El.inputValor.value);
    const formaPagamento = El.selectFormaPagamento.value;
    ...
    // Regra: desconto entre 5% e 10% para dinheiro em espécie ou PIX
    if (formaPagamento === 'dinheiro' || formaPagamento === 'pix') {
        const porcentagemDesconto = parseFloat(El.inputDesconto.value);

        if (isNaN(porcentagemDesconto) || porcentagemDesconto < 5 || porcentagemDesconto > 10) {
            alert("Para pagamento em dinheiro ou pix, o desconto deve ser um valor entre 5% e 10%.");
            El.inputDesconto.focus();
            return;
        }

        const valorDesconto = valorOriginal * (porcentagemDesconto / 100);
        valorFinal = valorOriginal - valorDesconto;

        const labelPagamento = formaPagamento === 'dinheiro' ? 'Dinheiro' : 'Pix';
        textoDesconto = ` <small style="color: #2f855a;">(-${porcentagemDesconto}% ${labelPagamento}: -${formatarMoeda(valorDesconto)})</small>`;
    }

    const novoItem = document.createElement('li');
    novoItem.innerHTML = `<span>${nomeServico} <small style="color: #718096;">(${formaPagamento.toUpperCase()})</small></span>${textoDesconto} <strong>${formatarMoeda(valorFinal)}</strong>`;
    El.listaServicos.appendChild(novoItem);

    somaTotal += valorFinal;
    El.inputTotalGeral.value = formatarMoeda(somaTotal);

    // Limpa os inputs para a próxima inserção
    El.inputValor.value = '';
    El.selectServico.value = '';
    El.selectFormaPagamento.value = '';
    El.inputDesconto.value = '';
    El.inputDesconto.disabled = true;
    El.selectServico.focus();
}
...
function limparFormulario() {
    ...
    if (El.listaServicos) El.listaServicos.innerHTML = '';
    if (El.inputTotalGeral) El.inputTotalGeral.value = 'R$ 0,00';
    somaTotal = 0;
    if (El.inputDesconto) El.inputDesconto.disabled = true;
}
